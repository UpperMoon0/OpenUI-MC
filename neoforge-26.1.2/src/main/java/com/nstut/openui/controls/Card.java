...
public class Card extends UIComponent {
    ...
    public int preferredWidth(Font font) {
        ...
        for (UIComponent child : children) {
            maxChildW = Math.max(maxChildW, child.preferredWidth(font));
        }
        return maxChildW + pad * 2;
    }
    ...
    public int preferredHeight(Font font) {
        ...
        int totalChildH = 0;
        for (UIComponent child : children) {
            totalChildH += child.preferredHeight(font);
        }
        return totalChildH + pad * 2;
    }
    ...
    public void layout(int x, int y, int availableWidth, int availableHeight) {
        ...
        int innerX = x + pad;
        int innerY = y + pad;
        int innerW = Math.max(0, availableWidth - pad * 2);
        int innerH = Math.max(0, availableHeight - pad * 2);
        for (UIComponent child : children) {
    ...
    public void render(GuiGraphicsExtractor g, Font font, int mx, int my, float pt) {
        ...
        int hoverBg = colors.surfaceVariant();
        int bg = UiRender.mix(baseBg, hoverBg, eased);
        ...
        if (elevated) {
            UiRender.shadow(g, x, y, width, height, radius);
        }
    ...
    public boolean mouseClicked(double mx, double my, int btn) {
        if (clickable && btn == 0 && isHovered() && onClick != null) {
            onClick.run();
            return true;
        }
    ...
    public boolean keyPressed(int key, int scanCode, int modifiers) {
        if (clickable && isFocused() && (key == 257 || key == 32) && onClick != null) {
            onClick.run();
            return true;
        }
