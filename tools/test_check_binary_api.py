...
"""
    ...
    def test_protected_member_break_is_detected_from_real_classfiles(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            _, _, baseline_members = class_api(shutil.which('javap'), baseline_jar, 'A')
            _, _, candidate_members = class_api(shutil.which('javap'), candidate_jar, 'A')
            removed = baseline_members - candidate_members
            self.assertTrue(any(declaration.startswith('protected int state') for declaration, _ in removed))
            self.assertTrue(any(declaration.startswith('protected void hook') for declaration, _ in removed))
