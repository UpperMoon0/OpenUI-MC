...
public class UiInspector extends UIComponent {
    ...
    public void render(GuiGraphics g, Font font, int mx, int my, float pt) {
        ...
        // Pick up hosts mounted after the inspector was opened. Profiling is
        // disabled and cleared again when the inspector closes.
        setProfiling(true);
    ...
    private void renderInfoPanel(GuiGraphics g, Font font, int mx, int my) {
        ...
        if (font == null) return;
        UiRender.text(g, font, "UI Inspector / Devtools", panelX + 6, panelY + 6, 0xFF5B8DF2);

        int count = countMounted(runtime.root());
        UiRender.text(g, font,
                "Mounted: " + count + "  Anim: " + runtime.animations().activeCount() + "  Overlays: " + runtime.overlays().size(),
                panelX + 6, panelY + 18, 0xFF8A8A9E);
        ...
        try {
            for (int i = startIdx; i < Math.min(entries.size(), startIdx + maxTreeLines); i++) {
                ...
                UiRender.text(g, font, trim(line, 47), panelX + 6, rowY, col);
            }
        } finally {
            ClipStack.pop(g);
        }
    ...
    private static void drawLine(GuiGraphics g, Font font, int panelX, int y, String text, int color) {
        UiRender.text(g, font, trim(text, 47), panelX + 6, y, color);
    }
